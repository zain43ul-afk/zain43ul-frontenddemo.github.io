...
export default function Home() {
  ...
  const drag = useRef({ active: false, moved: false, captured: false, pointerId: -1, startX: 0, currentX: 0 });
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    drag.current = { active: true, moved: false, captured: false, pointerId: event.pointerId, startX: event.clientX, currentX: event.clientX };
  };
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (Math.abs(distance) > 8 && !drag.current.moved) {
  ...
  const handlePointerUp = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (wasCaptured && event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId);
        ...
        <div className="project-swiper">
          ...
          >
            {visible.map((project, index) => {
              ...
              return (
                <Link className={`portfolio-card slider-card ${position} card-${project.slug} ${project.image ? "has-image" : "has-ui"}`} href={`/showcase/${project.slug}`} key={project.slug} style={{ "--accent": project.accent } as React.CSSProperties} draggable={false} tabIndex={index === activeSlide ? 0 : -1} aria-hidden={index !== activeSlide} onClick={(event) => { if (index !== activeSlide) { event.preventDefault(); setActiveSlide(index); } }}>
